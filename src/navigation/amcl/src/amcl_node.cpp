...
static bool get_pose_from_rviz_flag = false;
...
class AmclNode
{
  ...
  private:
    ...
    void mapname_callback(const std_msgs::StringConstPtr& msg);
        ...
        first_reconfigure_call_(true)
{
  ...
  laser_scan_filter_->registerCallback(boost::bind(&AmclNode::laserReceived,
                                                   this, _1));
  initial_pose_sub_ = nh_.subscribe("initialpose", 1, &AmclNode::initialPoseReceived, this);
...
AmclNode::checkLaserReceived(const ros::TimerEvent& event)
{
  ...
  double delta_t = ros::Time::now().toSec() - last_laser_received_ts_.toSec();//lyy
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  pf_vector_t pose;//comment by lyy
  ...
  double range_min,angle_min,angle_increment;//lyy
  // If the robot has moved, update the filter
  if(lasers_update_[laser_index] || (get_pose_from_rviz_flag&&!is_converged))//lyy: resampling
  // if(lasers_update_[laser_index])
  {
    AMCLLaserData ldata;//lyy
    ...
    // lyy: resampling untill converged
    is_converged = set->converged;
    if(is_converged) get_pose_from_rviz_flag = false;
  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0)
    {
      ...
      pose = hyps[max_weight_hyp].pf_pose_mean;
  ...
  s_data.sensor = lasers_[laser_index];
  s_data.range_count = laser_scan->ranges.size();

  if(laser_max_range_ > 0.0)
    s_data.range_max = std::min(laser_scan->range_max, (float)laser_max_range_);
  else
    s_data.range_max = laser_scan->range_max;

  s_data.ranges = new double[s_data.range_count][2];
  ...
  for(int i=0;i<s_data.range_count;i++)
  {
    // amcl doesn't (yet) have a concept of min range.  So we'll map short
    // readings to max range.
    if(laser_scan->ranges[i] <= range_min)
      s_data.ranges[i][0] = s_data.range_max;
    else
      s_data.ranges[i][0] = laser_scan->ranges[i];
    // Compute bearing
    s_data.ranges[i][1] = angle_min +
            (i * angle_increment);
  }
  // double score = lasers_[laser_index]->LikelihoodFieldModel_m(&s_data, pose);// lyy
  // ROS_INFO_THROTTLE(0.1,"laser points num: %d, amcl score: %f",s_data.range_count,score);// lyy
}
...
{
  get_pose_from_rviz_flag = true;
  printf("Get pose from rviz once!\n");
